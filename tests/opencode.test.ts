...
describe('ensureOpenCodeConfig', () => {
  ...
  it('does not overwrite existing opencode.json or prompt files', () => {
    const repoPath = makeRepoPath();
    ...
    expect(existsSync(path.join(repoPath, '.opencode', 'agent', 'coder9b.md'))).toBe(true);
    expect(existsSync(path.join(repoPath, '.opencode', 'agent', 'reviewer.md'))).toBe(true);
    expect(result.created).toEqual(['.opencode/agent/coder9b.md', '.opencode/agent/reviewer.md']);
    expect(result.skipped).toEqual(['.opencode/opencode.json', '.opencode/agent/planner.md']);
  });
